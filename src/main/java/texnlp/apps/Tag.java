...
public class Tag {
	...
	try {
		...
		TagResults results = 
		    new TagResults("dev", new File(devFile), outputDir, true, 
			    true, taggerOptions.getFormat());
	...
	try {
		...
		TagResults results = 
		    new TagResults("eval", new File(evalFile), outputDir, false, 
			    false, taggerOptions.getFormat());
		tagger.tagFile(results);
		results.close();
		final String resultsOutput = results.getOutput().toString();
		Score.score(resultsOutput, evalFile, trainingFile, 
			taggerOptions.getFormat(), 1);
	    }
	} catch (IOException e) {
	...
	options.addOption( "s", "tagset", true, "a prespecified tagset (e.g., for use with cutoffs to ignore low frequency tags)" );
	...
	options.addOption( "b", "beta", true, "use variable width beam during viterbi decoding using beta*maxprob as a threshhold (default = .001)" );
	options.addOption( "c", "td-cutoff", true, "use variable width cutoff in tag dictionary (default = 0.0)" );
	options.addOption( "u", "unconstrained", false, "don't be constrained by tag dictionary when outputting multiple tags (works for MEL only)" );
