...
GETOPTS=`getopt -o $SHORTOPTS -l $LONGOPTS -n 'ctag.sh' -- "$@"`
eval set -- "$GETOPTS"

while true ; do
    case $1 in
        -h|--help) cat <<EOF ; exit 0 ;; 
usage: ctag.sh options
 -a,--mpi <arg>              
 -b,--beta <arg>             
 -c,--td-cutoff <arg>        
...
# Run C&C training        
echo "Training..."
echo "$CANDC/bin/$TRAIN_BIN --model $MODEL_DIR --comment \"Foo\" --input $TRAIN_FILE --solver bfgs $SMOOTH $IFMT"
...
if [ $DEV_FILE ] ; then
    ...
    CMD="$CANDC/bin/$TEST_BIN --model $MODEL_DIR --maxwords 5000 --input $DEV_FILE $IFMT $OFMT > $RESULT_FILE"
    echo $CMD && eval $CMD
