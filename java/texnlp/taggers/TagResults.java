...
public class TagResults {
    ...
    protected enum OutputType { PIPE, CONLL, HASH; };
    protected OutputType format = OutputType.PIPE;
    
    public TagResults(String name, File input, File outputDir,
	    boolean savePerplexity, boolean saveEntropy, String format) {
	...
	try {
		...
		taggedOutput = new BufferedWriter(new OutputStreamWriter(
	...
	if (savePerplexity) {
	    try {
		if (input.getName().endsWith(".gz"))
		    perplexity = new BufferedWriter(new OutputStreamWriter(
  	                           new GZIPOutputStream(new FileOutputStream(
				     new File(outputDir, name + ".perplexity.txt.gz")))));
		else
		    perplexity = new BufferedWriter(new FileWriter(
                                   new File(outputDir, name + ".perplexity.txt")));
	...
	if (saveEntropy) {
	    try {
		if (input.getName().endsWith(".gz"))
		    entropy = new BufferedWriter(new OutputStreamWriter(
  	                           new GZIPOutputStream(new FileOutputStream(
				     new File(outputDir, name + ".entropy.txt.gz")))));
		else
		    entropy = new BufferedWriter(new FileWriter(
                                   new File(outputDir, name + ".entropy.txt")));
    ...
    public String[][] addTagsIte(String[] toks,
            Vector<String[]> tags) throws IOException {
