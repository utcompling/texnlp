...
/**
 * Count events for HMMs. Doubles are used so that partial counts
 * obtained from Baum-Welch can be added.
 *
...
public class Counts {
    ...
    public Counts (int _numTags, boolean tagdictTraining, 
		   boolean dirichletTransition, boolean dirichletEmission) {
	this(_numTags);
    ...
    public Counts copy () {
	//return new Counts(numTags, tagdictTraining, useDirichletTransition, useDirichletEmission);

	Counts ccopy = 
	    new Counts(numTags, tagdictTraining, useDirichletTransition, useDirichletEmission);
	...
	if (startWithPrior) {
		...
		if (useDirichletTransition)
		    probs[i] = 
			MathUtil.elog(vbF(priorMultiplier*tprior.tInitial[i] + dirichletAlpha)
		...
		if (useDirichletTransition)
		    probs[i] = 
			MathUtil.elog(vbF(c_tInitial[i] + dirichletAlpha)
				      /vbF(total + dirichletAlpha*numTags));
		else
		    probs[i] = 
			MathUtil.elog((c_tInitial[i] + lambda_tt[i]*backoff[i])
				      /(total + lambda_tt[i]));
	    }
	}
	...
	if (startWithPrior) {
		...
		if (useDirichletTransition)
		    probs[i] = 
			MathUtil.elog(vbF(priorMultiplier*tprior.tFinal[i] + dirichletAlpha)
				      /vbF(priorMultiplier + dirichletAlpha*numTags));
		else
		...
		if (useDirichletTransition)
		    probs[i] = 
			MathUtil.elog(vbF(c_tFinal[i] + dirichletAlpha)
				      /vbF(total + dirichletAlpha*numTags));
		else
		    probs[i] = 
			MathUtil.elog((c_tFinal[i] + lambda_tt[i]*backoff[i])
				      /(total + lambda_tt[i]));
	    }
	}
	...
	if (startWithPrior) {
		...
		for (int j=0; j<numTags; j++) {
			...
			probs[i][j] = 
			    MathUtil.elog(
			        vbF(priorMultiplier*tprior.tt[i][j] + dirichletAlpha)
				/vbF(priorMultiplier + dirichletAlpha*numTags));
		    else
		...
		for (int j=0; j<numTags; j++) {
		    if (useDirichletTransition)
			probs[i][j] = 
			    MathUtil.elog(vbF(c_tt[i][j] + dirichletAlpha)
		    ...
		    //System.out.println(i + "::"+j+": "+Math.exp(probs[i][j]));
		    }
		}
	    }
	}
	...
	TObjectDoubleHashMap<String> unigramSmoothed = 
	    new TObjectDoubleHashMap<String>(c_w.size());
	...
	THashMap<String, TLinkedList<IntDoublePair>> probs = 
	    new THashMap<String, TLinkedList<IntDoublePair>>(c_w.size());
	...
	for (int i=0; i<numTags; i++) {
		...
		unknownProb = 
		    MathUtil.elog((lambda_tw[i]/(numWords+numTypes)) / (emissionTotals[i]+lambda_tw[i]));
	    
	    //System.out.println(i + " => " + MathUtil.eexp(unknownProb) + " :: " + numEmissions + " :: " + emissionTotals[i] + "\t:: " + lambda_tw[i]);
		...
		double p;
		if (useDirichletEmission)
		    p = MathUtil.elog(vbF(iter.value() + localDA)
				      /vbF(emissionTotals[i] + localDA*numTypes));
		else
	...
	return Math.log(v) 
	    + 0.04167*Math.pow(v,-2) 
	    - 0.00729*Math.pow(v,-4)
	    + 0.00384*Math.pow(v,-6)
    ...
    public void createTransitionPrior (String[] stateNames, 
				       TObjectIntHashMap<String> states, 
				       double amount) {
