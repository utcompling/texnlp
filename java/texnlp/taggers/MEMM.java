...
public class MEMM extends MarkovModel {
	...
	if (contextGenOption.equals("word")) 
	    ...
	    throw new RuntimeException("Unknown word context generator: " + 
		    contextGenOption);
    }

    protected final double[][] getTransitionLogProbs(int[] iStates, int[] jStates, 
    ...
    protected void addSpecificContexts(String identifier, 
				       String[] wordContexts,
				       THashSet<String> fullContexts) {
	...
	try {
	    ...
	    // now observe events
	    mInitial = new TadmClassifier("initial", "tao_lmvm", numStates, 
					  maxIterations, variance, numMachines);
	    mFinal = new TadmClassifier("final", "tao_lmvm", numStates, 
					maxIterations, variance, numMachines);
	    mTransition = new TadmClassifier("transition", "tao_lmvm", numStates, 
					     maxIterations, variance, numMachines);
...
//			String output = sequence[tokenID][0];
		    ...
		    mFinal.addEvent(prevStateID, 
				    new Context(cachedWordFeatures[sequence.length-1]));
