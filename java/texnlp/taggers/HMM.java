...
/**
 * A Hidden Markov Model, trained on tagged and untagged
 * sentences. Constructs a tag dictionary from the tagged data.
 *
...
public class HMM extends MarkovModel {
    ...
    // The minimum change in log probability between iterations for EM to keep going.
	...
	for (int tokenID=1; tokenID<numTokens; tokenID++) {
		...
		int bestPrevState = -1;
		    ...
		    final double pathLogProb = 
			prevPathLogProb + transitionLogProb + emissionLogProb;
		    
		// We can skip the use of MathUtil.elogProduct since
		// neither of these values can be zero. This is what it
		// would be otherwise:
		//MathUtil.elogProduct(viterbi[tokenID-1][iStateID],pTransition[iStates[i]][stateID], current.doubleValue);
		    if (pathLogProb > max) {
			max = pathLogProb;
	    ...
	    //System.out.println(tokens[tokenID] + ": " + StringUtil.join(validTags[tokenID]));
    ...
    protected double[][] getTransitionLogProbs(int[] iStates, int[] jStates, 
	...
	for (int j=validStates.size(); j>0; j--) {
		...
		probs[iStates[i]][stateID] = 
		    pTransition[iStates[i]][stateID] + current.doubleValue;

	    // We can skip the use of MathUtil.elogProduct since
	    // neither of these values can be zero. This is what it
	    // would be otherwise:
	    //MathUtil.elogProduct(pTransition[iStates[i]][stateID], current.doubleValue);
	...
	try {
	    ...
	    Counts cEmpty = 
		new Counts(numStates, tagdictTraining, dirichletTransition, dirichletEmission);
	    Counts cOrig = 
		new Counts(numStates, tagdictTraining, dirichletTransition, dirichletEmission);
	    Counts cTotal;
	...
	try {
		...
		// Do first block if there are multitags, otherwise do the normal thing
	...
	try {
		...
		for (int itemID=0; itemID<numSequences; itemID++) {
		    totalProbForK += emForSequence(allSequences[itemID], cnew, 
						   possibleTags[itemID], probWordGivenTag[itemID]);
		}
		...
		if (averageLogProb - previousLogProb < tolerance) {
			...
			System.out.println("DONE: Change in average log probability is less than " 
					   + tolerance);
		    }
		    break;
		}
		...
		for (int itemID=0; itemID<numSequences; itemID++) {
		    String[] tokens = allSequences[itemID];
		    for (int tokenID=0; tokenID<tokens.length; tokenID++) {
			//TLinkedList<IntDoublePair> valid = pEmission.get(tokens[tokenID]);
			TLinkedList<IntDoublePair> valid = pEmission.getStrict(tokens[tokenID]);
			if (null != valid) {
    ...
    private double emForSequence (String[] tokens, Counts cnew, 
				  int[][] possibleTags, double[][] pWGT) {
	...
	for (int jCounter=0; jCounter<possibleTags[0].length; jCounter++) {
	    ...
	    forward[0][jStateID] = 				 
		MathUtil.elogProduct(pInitial[jStateID], pWGT[0][jCounter]);
	}
		...
		final int jStateID = possibleTags[tokenID][jCounter];
		    ...
		    forward[tokenID][jStateID] =
			MathUtil.elogSum(forward[tokenID][jStateID],
			  MathUtil.elogProduct(forward[iTokenID][iStateID], 
					       pTransition[iStateID][jStateID],
					       pWGT[tokenID][jCounter]));
		}
	    }
    	}
	...
	for (int jCounter=0; jCounter<possibleTags[lastTokenID].length; jCounter++) {
	    final int jStateID = possibleTags[lastTokenID][jCounter];
    	    logForwardProb =
		MathUtil.elogSum(logForwardProb, 
				 MathUtil.elogProduct(forward[lastTokenID][jStateID],
						      pFinal[jStateID]));
	}
	...
	for (int jCounter=0; jCounter<possibleTags[lastTokenID].length; jCounter++) {
	    final int jStateID = possibleTags[lastTokenID][jCounter];
	    final double prob_t_at_final = 
		MathUtil.eexp(MathUtil.elogProduct(forward[lastTokenID][jStateID],
						   backward[lastTokenID][jStateID],
						   -logForwardProb,
						   lambda));
		...
		final double prob_t_at_j = 
		    MathUtil.eexp(MathUtil.elogProduct(forward[jTokenID][jStateID],
						       backward[jTokenID][jStateID],
						       -logForwardProb,
						       lambda));
		...
		cnew.increment(tokens[jTokenID], prob_t_at_j);
		    ...
		    final double prob = MathUtil.elogProduct(pTransition[iStateID][jStateID],
							     pWGT[jTokenID][jCounter]);

		    backward[tokenID][iStateID] =
			MathUtil.elogSum(backward[tokenID][iStateID], 
				 MathUtil.elogProduct(backward[jTokenID][jStateID], prob));

		    cnew.increment(iStateID, jStateID,
			  MathUtil.eexp(MathUtil.elogProduct(forward[tokenID][iStateID],
							     prob,
							     backward[jTokenID][jStateID],
							     -logForwardProb,
							     lambda)));
	...
	for (int jCounter=0; jCounter<possibleTags[0].length; jCounter++) {
	    final int jStateID = possibleTags[0][jCounter];
    	    ...
    	    logBackwardProb =
		MathUtil.elogSum(logBackwardProb,
				 MathUtil.elogProduct(pInitial[jStateID],
						      pWGT[0][jCounter],
						      backward[0][jStateID]));

	    final double prob_t_at_initial = 
		MathUtil.eexp(MathUtil.elogProduct(forward[0][jStateID],
						   backward[0][jStateID],
						   -logForwardProb,
						   lambda));
