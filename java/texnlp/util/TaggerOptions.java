...
public class TaggerOptions {
	...
	for (Option option: cline.getOptions()) {
	    ...
	    switch (option.getOpt().charAt(0)) {
	        case 'f': setFormat(value);     break;
	        case 'w': setContextGen(value); break;
	    	case 'i': numIterations     = Integer.parseInt(value);   break;
	    	case 'a': numMachines       = Integer.parseInt(value);   break;
	    	case 'p': priorAmount       = Double.parseDouble(value); break;
	    	case 'b': logBeta           = Math.log(Double.parseDouble(value)); break;
	    	case 'c': cutoffTD          = Double.parseDouble(value); break;
	    	case 'q': lambda            = Double.parseDouble(value); break;
	    	case 'm': modelType              = value; break;
	    	case 't': taggedFile             = value; break;
	    	case 'r': rawFile                = value; break;
	    	case 'd': devFile                = value; break;
	    	case 'e': evalFile               = value; break;
	    	case 'l': machineFile            = value; break;
	    	case 'o': outputDir              = new File(value); break;
	    	case 's': tagsetFile             = value; break;
	    	case 'g': tagdictTraining        = true; break;
	    	case 'u': unconstrainedByTagdict = true; break;
	    	case 'n': multitag               = true; break;
	    	case 'j': dirichletTransition    = true; break;
	    	case 'k': dirichletEmission      = true; break;
	    }
	}
	...
	if (outputDir == null) {
	    ...
	    // This removes the created file so we can recreate it as a directory.
	    outputDir.delete(); 
	...
	if (!outputDir.mkdirs()) {
	    System.out.println("************************************************************************");
	    System.out.println("Unable to create directory \"" + outputDir + "\" for output.\nIt probably already exists -- choose a unique name.");
	    System.out.println("************************************************************************");
	    System.exit(0);
	}
    }
