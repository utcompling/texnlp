...
public class TadmClassifier {
    ...
    public static String PATH_TO_ESTIMATE_EXECUTABLE
	= Constants.TADM_HOME+"/bin/tadm";
    ...
    public TadmClassifier (String _name, String _method, 
			   int _numLabels, int _iterations, 
			   double _variance, int _numMachines) {
	name = _name;
	method = _method;
	...
	IOUtil.runCommand(
	  mpiOption
          + PATH_TO_ESTIMATE_EXECUTABLE
	  + " -monitor"
	  + " -events_in " + eventsFile.getAbsolutePath()
	  + " -params_out " + paramsFile.getAbsolutePath()
	  + iterationOption
	  + " -method " + method
	  + varianceOption
	  + " -frtol 1e-10");
	...
	try {
		...
		new BufferedWriter(new OutputStreamWriter(
		    new GZIPOutputStream(new FileOutputStream(eventsFile))));
